...
echo "-> Attaching eBPF dropper to allow only port $PORT_ALLOW"
sudo ./bin/dropper --cgroup "$CGROUP_PATH" --egress --port "$PORT_ALLOW" --create --auto-mount &
DROPPER_PID=$!
...
echo "===> Step 6: Test with specific process targeting"
...
if [ -n "$NGINX_PID" ]; then
    ...
    # Test that nginx is now affected by the dropper
    echo "-> Testing nginx process in cgroup (should be blocked on port $PORT_BLOCK)"
    test_http_endpoint "http://127.0.0.1:$PORT_BLOCK" "fail" "Nginx in cgroup on blocked port"
    
    echo "-> Testing nginx process in cgroup (should work on port $PORT_ALLOW)"
    test_http_endpoint "http://127.0.0.1:$PORT_ALLOW" "success" "Nginx in cgroup on allowed port"
else
...
echo "===> Step 8: Test different HTTP methods and endpoints"
...
echo "✅ Works with HTTP clients (curl)"#!/usr/bin/env bash
set -euo pipefail
